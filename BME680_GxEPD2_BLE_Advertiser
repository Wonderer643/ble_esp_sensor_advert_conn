...
//    breathVocAccuracyCharacteristic.setValue(ACCURACY(iaqSensor.breathVocAccuracy));
  ...
  BLEAdvertising *advertising = bsecServer->getAdvertising();
  ...
  char sensor_chars[sizeof(sensor_data_t)];
//  advertisingData.setCompleteServices((BLEUUID)uint16_t(0x2A6E));
//  sprintf(s2tmp,"%+#.2f",iaqSensor.temperature);
  sensor.temp=iaqSensor.temperature;
  sensor.hum=iaqSensor.humidity;
  sensor.pres=iaqSensor.pressure * 0.007500616827;
  sensor.voc=iaqSensor.breathVocEquivalent;
  sensor.acc=iaqSensor.breathVocAccuracy;
  memcpy(sensor_chars,&sensor,sizeof(sensor_data_t));
  advertisingData.setServiceData((BLEUUID)uint16_t(0X2AF6),sensor_chars);

//  advertisingData.setCompleteServices((BLEUUID)uint16_t(0x2A6F));
//  sprintf(s2tmp,"%#.2f",iaqSensor.humidity);
//  advertisingData.setServiceData((BLEUUID)uint16_t(0x2A6F),s2tmp);

//  advertisingData.setCompleteServices((BLEUUID)uint16_t(0x2A6D));
//  sprintf(s2tmp,"%#.0f",iaqSensor.pressure * 0.007500616827);
//  advertisingData.setServiceData((BLEUUID)uint16_t(0x2A6D),s2tmp);
